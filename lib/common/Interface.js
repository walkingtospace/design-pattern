...
Interface.prototype.ensureImplements = function (object) {
	...
	if ( length < 2 ) {
		throw new Error("[Interface.ensureImplements] Argument length is expected 2");
	}

	for ( var i=1; i<length; i++) {	//argument[0] == object, argument[1,2,3, ...] == origin interfaces
		...
		var methodLength = interfaces.methods.length;
		for ( var j=0 ; j<methodLength ; j++ ) {
			var method = interfaces.methods[j];
			if (!object[method] || typeof object[method] !== 'function') {
				throw new Error("[Interface.ensureImplements] object " + "does not implement the " + interfaces.name + " interface. Method " + method + " was not found.");
			}
		}
	}
};